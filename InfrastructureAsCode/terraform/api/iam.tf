...
resource "aws_iam_policy" "my_api_execution_policy" {
  name        = "${var.service_name}-execution-policy"
  description = "This policy allows ECS to deploy"
 
  policy = data.aws_iam_policy_document.my_api_execution_policy_doc.json
}


data "aws_iam_policy_document" "my_api_execution_policy_doc" { //TODO: Tighten Permissions
  statement {
    effect  = "Allow"
    actions = [
...
resource "aws_iam_role_policy_attachment" "execution-attach" {
  ...
  policy_arn = aws_iam_policy.my_api_execution_policy.arn
}

// Task Role

resource "aws_iam_role" "task_role" {
  name               = "${var.service_name}_api_task_role"
  assume_role_policy = data.aws_iam_policy_document.assume_role_policy.json
}
